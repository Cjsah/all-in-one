    ...
    public static void onInitialize() {
        CommandRegistrationCallback.EVENT.register(((dispatcher, dedicated) -> dispatcher.register(
                CommandManager.literal("plan").then(CommandManager.literal("add").then(CommandManager.argument("plan", StringArgumentType.greedyString()).executes(context -> {
                    AllInOneMod.getState(context).addPlan(context.getSource(), StringArgumentType.getString(context, "plan"));
                    ...
                    AllInOneMod.getState(context).removePlan(context.getSource(), IntegerArgumentType.getInteger(context, "index"));
                    return Command.SINGLE_SUCCESS;
                    ...
                    AllInOneMod.getState(context).modifyPlan(context.getSource(), IntegerArgumentType.getInteger(context, "index"), StringArgumentType.getString(context, "content"));
                    return Command.SINGLE_SUCCESS;
                })))).then(CommandManager.literal("complete").then(CommandManager.argument("index", IntegerArgumentType.integer(0)).executes(context -> {
                    AllInOneMod.getState(context).completePlan(context.getSource(), IntegerArgumentType.getInteger(context, "index"), true);
                    return Command.SINGLE_SUCCESS;
                }).then(CommandManager.literal("false").executes(context -> {
                    AllInOneMod.getState(context).completePlan(context.getSource(), IntegerArgumentType.getInteger(context, "index"), false);
                    return Command.SINGLE_SUCCESS;
                })))).then(CommandManager.literal("list").then(CommandManager.literal("all").executes(context -> {
                    AllInOneMod.getState(context).listAllPlan(context.getSource());
                    return Command.SINGLE_SUCCESS;
                })).then(CommandManager.literal("completed").executes(context -> {
                    AllInOneMod.getState(context).listPlan(context.getSource(), true);
                    return Command.SINGLE_SUCCESS;
                })).then(CommandManager.literal("unfinished").executes(context -> {
                    AllInOneMod.getState(context).listPlan(context.getSource(), false);
                    return Command.SINGLE_SUCCESS;
                })))
        )));
    }
